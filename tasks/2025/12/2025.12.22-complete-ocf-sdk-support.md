...
Add SDK support for all remaining OCF object types and establish a robust testing strategy using a hybrid of unit tests (mocks) and integration tests (LocalNet).
...
- **OCF schema validation** (`test/utils/ocfSchemaValidator.ts`) - Validates against official schemas
...
- [ ] Other transactions (TX_STOCK_REISSUANCE, TX_STOCK_CONSOLIDATION, TX_EQUITY_COMPENSATION_REPRICING)
