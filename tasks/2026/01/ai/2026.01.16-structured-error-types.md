...
- [ ] Create `src/errors/OcpError.ts` base class
- [ ] Create `src/errors/OcpValidationError.ts`
- [ ] Create `src/errors/OcpContractError.ts`
- [ ] Create `src/errors/OcpNetworkError.ts`
- [ ] Create `src/errors/OcpParseError.ts`
- [ ] Create `src/errors/index.ts` barrel export
- [ ] Define error codes in `src/errors/codes.ts`

### Phase 2: Update Validation Code

- [ ] Update `stakeholderDataToDaml.ts` to use `OcpValidationError`
- [ ] Update `stockIssuanceDataToDaml.ts` to use `OcpValidationError`
- [ ] Update other entity converters
- [ ] Update `normalizeNumericString()` to use structured errors

### Phase 3: Update SDK Methods

- [ ] Update `CapTableBatch.execute()` to wrap errors
- [ ] Update `get*AsOcf()` methods to wrap parse errors
- [ ] Update network calls to wrap connectivity errors

### Phase 4: Documentation

- [ ] Add error handling section to documentation
- [ ] Add examples for catching specific error types
