...
describe('DAML ↔ Native Round-Trip', () => {
...
- [x] Add `fast-check` as dev dependency: `npm install --save-exact --save-dev fast-check`
- [x] Create `test/property/` directory for property tests
- [x] Add Jest configuration for property tests (existing config already works)

### Phase 2: Core Conversion Tests

- [x] Create `test/property/typeConversions.property.test.ts`
- [x] Add monetary round-trip tests
- [x] Add date round-trip tests
- [x] Add numeric string normalization tests

### Phase 3: Entity Conversion Tests

- [x] Add address round-trip tests
- [ ] Add stakeholder data round-trip tests (deferred - requires more complex conversion functions)
- [ ] Add stock class data round-trip tests (deferred - requires more complex conversion functions)

### Phase 4: Integration with CI

- [x] Add property tests to test suite (included in `npm test`)
- [x] Configure appropriate iteration counts (500 runs for core tests, 100-300 for edge case tests)
- [x] Document how to run extended property tests locally
...
| 2026-01-20 | Implemented property-based testing with fast-check 4.5.3. Added core type conversion and address tests. | --  |
