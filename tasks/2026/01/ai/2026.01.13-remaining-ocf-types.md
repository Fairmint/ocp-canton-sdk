...
**Status**: Open
**Priority**: Medium
**Parent**: [Milestone 2: OCP SDK Implementation](../../../2025/12/2025.12.17-milestone-2-ocp-sdk-implementation.md)

## Overview

Implement SDK support for all remaining OCF object types that are supported in the DAML implementation but not yet exposed in the SDK.

## Current State

The DAML `OcfCreateData` union type supports **48 OCF object types**. The SDK currently implements **21 types**, leaving **27 types** to be added.
...
Continue the current pattern of creating individual `create*`, `get*AsOcf`, and `delete*` functions for each type.
...
Implement remaining types via the batch `UpdateCapTable` API as specified in [ADR-001](../../../../adr/001-batch-cap-table-updates.md).
