...
Identify and document refactoring opportunities, API improvements, and testing gaps in the OCP Canton SDK to improve developer experience, code quality, and test coverage confidence.

## Context

The OCP Canton SDK has evolved significantly since its creation. With LocalNet (`cn-quickstart`) now available for comprehensive testing, we have an opportunity to:
...
- [ ] **Eliminate `unknown[]` for trigger arrays**
  - Files: `src/types/native.ts` lines 730, 771-777
  - Issue: `conversion_triggers: unknown[]`, `exercise_triggers: unknown[]`
  - Solution: Define proper types for OCF trigger schemas

- [ ] **Remove `@ts-expect-error` comments**
  - File: `src/utils/createOcfObject.ts` lines 237, 246
  - Cause: Type mismatch for warrant/convertible issuance data
  - Solution: Align native types with SDK input types
...
#### 2.1 Define Trigger Types
Research OCF schema for conversion/exercise triggers and define proper TypeScript types.

**Files to modify:**
- `src/types/native.ts` — Add trigger type definitions
- `src/functions/OpenCapTable/warrantIssuance/createWarrantIssuance.ts`
- `src/functions/OpenCapTable/convertibleIssuance/createConvertibleIssuance.ts`
- `src/utils/createOcfObject.ts` — Remove `@ts-expect-error`
...
### Phase 2: Type Safety
- [ ] Define OCF trigger types in `native.ts`
- [ ] Update warrant/convertible functions
- [ ] Remove `@ts-expect-error` from `createOcfObject.ts`
