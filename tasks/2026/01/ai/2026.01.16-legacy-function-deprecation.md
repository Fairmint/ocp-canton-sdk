...
### Phase 1: Remove Legacy Exports (Complete)

- [x] Remove public exports of all `*DataToDaml` functions from index files (20 functions)
- [x] Functions remain private for internal use by batch API (`ocfToDaml.ts`)
- [x] Source files kept (not deleted) - required by internal `ocfToDaml.ts` dispatcher

**Note:** The source files (`create*.ts`, `*DataToDaml.ts`) cannot be deleted because they contain
the OCF-to-DAML conversion logic used internally by the batch API. The key achievement is removing
them from the public API - users can now only use the batch `UpdateCapTable` API.
...
| 2026-01-20 | Removed public exports of all 20 \*DataToDaml functions from index files | --  |
| 2026-01-20 | Source files retained for internal batch API use; Phase 1 complete       | --  |
