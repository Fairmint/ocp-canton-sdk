...
function securityExemptionToDaml(
  e: SecurityExemption
): Fairmint.OpenCapTable.Types.OcfSecurityExemption {
  return {
...
function shareNumberRangeToDaml(
  r: ShareNumberRange
): Fairmint.OpenCapTable.Types.OcfShareNumberRange {
  return {
    starting_share_number:
      typeof r.starting_share_number === 'number'
        ? r.starting_share_number.toString()
        : r.starting_share_number,
    ending_share_number:
      typeof r.ending_share_number === 'number'
        ? r.ending_share_number.toString()
        : r.ending_share_number,
  };
}
...
function stockIssuanceDataToDaml(
  d: OcfStockIssuanceData
): Fairmint.OpenCapTable.StockIssuance.OcfStockIssuanceData {
  ...
  return {
    ...
    stockholder_approval_date: d.stockholder_approval_date
      ...
      : null,
    consideration_text:
      d.consideration_text && d.consideration_text != '' ? d.consideration_text : null,
    ...
    share_numbers_issued: (d.share_numbers_issued || [])
      .filter(
        (range) => !(range.starting_share_number === '0' && range.ending_share_number === '0')
      )
    ...
    vestings: (d.vestings || []).map((v) => ({
      ...
      amount: typeof v.amount === 'number' ? v.amount.toString() : v.amount,
    })),
