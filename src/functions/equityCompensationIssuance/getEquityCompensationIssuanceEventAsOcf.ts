  ...
  const event: OcfEquityCompensationIssuanceEvent = {
    ...
    ...(d.base_price
      ? {
          base_price: {
            amount:
              typeof d.base_price.amount === 'number'
                ? String(d.base_price.amount)
                : d.base_price.amount,
            currency: d.base_price.currency,
          },
        }
      : {}),
    ...
    security_law_exemptions: (d.security_law_exemptions as any[]).map((ex: any) => ({
