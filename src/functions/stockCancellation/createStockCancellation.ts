...
export async function createStockCancellation(
  ...
  const createArguments = { context: { issuer: params.issuerParty, system_operator: systemOperator, featured_app_right: params.featuredAppRightContractDetails.contractId }, cancellation_data };

  const response = await client.submitAndWaitForTransactionTree({
    actAs: [params.issuerParty, systemOperator],
    commands: [ { CreateCommand: { templateId: Fairmint.OpenCapTable.StockCancellation.StockCancellation.templateId, createArguments: createArguments as any } } ],
...
export function buildCreateStockCancellationCommand(params: CreateStockCancellationParams & { systemOperator: string }): { command: Command; disclosedContracts: DisclosedContract[] } {
  const d = params.cancellationData;
  const cancellation_data: Fairmint.OpenCapTable.StockCancellation.OcfStockCancellationData = {
    ocf_id: d.ocf_id,
    date: dateStringToDAMLTime(d.date),
  ...
  const createArguments = { context: { issuer: params.issuerParty, system_operator: params.systemOperator, featured_app_right: params.featuredAppRightContractDetails.contractId }, cancellation_data };
  const command: Command = { CreateCommand: { templateId: Fairmint.OpenCapTable.StockCancellation.StockCancellation.templateId, createArguments: createArguments as any } };
