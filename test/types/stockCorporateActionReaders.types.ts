/** Compile-time contracts for stock corporate-action readers and converters. */

import type { DamlDataTypeFor, OcfReadDataTypeFor } from '../../src/functions/OpenCapTable/capTable/batchTypes';
import type {
...
import type { GetStockClassConversionRatioAdjustmentAsOcfResult } from '../../src/functions/OpenCapTable/stockClassConversionRatioAdjustment/getStockClassConversionRatioAdjustmentAsOcf';
import type {
  DamlStockClassSplitData,
  damlStockClassSplitToNative,
} from '../../src/functions/OpenCapTable/stockClassSplit/damlToStockClassSplit';
import type { GetStockClassSplitAsOcfResult } from '../../src/functions/OpenCapTable/stockClassSplit/getStockClassSplitAsOcf';
import type {
  DamlStockConsolidationData,
  damlStockConsolidationToNative,
} from '../../src/functions/OpenCapTable/stockConsolidation/damlToStockConsolidation';
import type { GetStockConsolidationAsOcfResult } from '../../src/functions/OpenCapTable/stockConsolidation/getStockConsolidationAsOcf';
import type {
  DamlStockReissuanceData,
  damlStockReissuanceToNative,
} from '../../src/functions/OpenCapTable/stockReissuance/damlToStockReissuance';
import type { GetStockReissuanceAsOcfResult } from '../../src/functions/OpenCapTable/stockReissuance/getStockReissuanceAsOcf';
import type {
  DamlStockRepurchaseData,
  damlStockRepurchaseToNative,
} from '../../src/functions/OpenCapTable/stockRepurchase/damlToOcf';
import type { GetStockRepurchaseAsOcfResult } from '../../src/functions/OpenCapTable/stockRepurchase/getStockRepurchaseAsOcf';
import type {
...
// @ts-expect-error corporate-action result wrappers are readonly
conversionRatioResult.contractId = 'other';
// @ts-expect-error corporate-action event fields are readonly
splitResult.event.id = 'other';
// @ts-expect-error corporate-action nested objects are recursively readonly
conversionRatioResult.event.new_ratio_conversion_mechanism.ratio.numerator = '4';
// @ts-expect-error corporate-action identifier tuples are recursively readonly
consolidationResult.event.security_ids.push('other');
// @ts-expect-error reissuance identifier tuples are recursively readonly
reissuanceResult.event.resulting_security_ids[0] = 'other';
// @ts-expect-error nested Monetary objects are recursively readonly
repurchaseResult.event.price.amount = '4';

// @ts-expect-error conversion-ratio DAML cannot be passed to the split converter
const wrongSplitDamlInput: Parameters<typeof damlStockClassSplitToNative>[0] = conversionRatioDaml;
// @ts-expect-error split DAML cannot be passed to the consolidation converter
const wrongConsolidationDamlInput: Parameters<typeof damlStockConsolidationToNative>[0] = splitDaml;
// @ts-expect-error consolidation DAML cannot be passed to the reissuance converter
const wrongReissuanceDamlInput: Parameters<typeof damlStockReissuanceToNative>[0] = consolidationDaml;
// @ts-expect-error reissuance DAML cannot be passed to the repurchase converter
const wrongRepurchaseDamlInput: Parameters<typeof damlStockRepurchaseToNative>[0] = reissuanceDaml;
// @ts-expect-error repurchase DAML cannot be passed to the conversion-ratio converter
const wrongConversionRatioDamlInput: Parameters<typeof damlStockClassConversionRatioAdjustmentToNative>[0] =
  repurchaseDaml;

// @ts-expect-error conversion-ratio adjustment cannot be used as a split
const wrongSplit: OcfStockClassSplit = conversionRatioResult.event;
// @ts-expect-error split cannot be used as a consolidation
const wrongConsolidation: OcfStockConsolidation = splitResult.event;
// @ts-expect-error consolidation cannot be used as a reissuance
const wrongReissuance: OcfStockReissuance = consolidationResult.event;
// @ts-expect-error reissuance cannot be used as a repurchase
const wrongRepurchase: OcfStockRepurchase = reissuanceResult.event;
// @ts-expect-error repurchase cannot be used as a conversion-ratio adjustment
const wrongConversionRatio: OcfStockClassConversionRatioAdjustment = repurchaseResult.event;
// @ts-expect-error canonical rounding is a closed union
